...
drinks %>% 
  ...
  labs(
    ...
    subtitle = "Per Serving Per Person",
    x = "Beer Servings",
    y = "Country"
  )
```
...
#no of rows in the unique dataset & storing in the variable nrow_unique
nrow_unique <- nrow(unique(movies))

#no of rows in the actual dataset & storing in the variable nrow_movies
...
-   Are there any missing values (NAs)? Are all entries distinct or are there duplicate entries?
    ...
    All the rows are distinct and no duplicates were found.
...
movies %>% 
  ...
  summarise(avg_gross = mean(gross),
            ...
            return_on_budget = sum(gross)/sum(budget),
            return_on_budget2 = mean(movie)) %>%
...
ggplot(movies_rating,mapping = aes(x = mean_rating)) + geom_histogram()
...
ggplot(movies, aes(x = cast_facebook_likes, y = gross)) +
  geom_point() +
  geom_smooth(method = "lm",se=FALSE,colour="black",span=0.8)

```

-   Facebook likes do not seem like a good indicator of the gross as there is no direct correlation as seen from the scatter plot.
...
-   
    ...
    The budget and gross do seem slightly correlated. The higher the budget, it is more likely that the gross may be higher.
...
-   The higher the rating the more will be the gross for the 'Action' & 'Adventure' genre movies.

-   The gross has a very less change with respect to rating for 'Horror' and 'Sci-Fi' genre movies.
...
# YOUR CODE GOES HERE
# sectorcount <-nyse %>% count(sector, sort=TRUE)

counter<-nyse %>% group_by(sector) %>% mutate(sector_count = count(sector)) 
counter <- arrange(counter, desc(sector_count))

ggplot(counter, aes(x=fct_reorder(sector,sector_count))) +
  geom_bar() 
...
# YOUR CODE GOES HERE
monthlystocks_summarised<-myStocks_returns_monthly %>% group_by(symbol) %>%
  summarise(minreturn = min(monthly_returns),
            maxreturn = max(monthly_returns),
            medianreturn = median(monthly_returns),
            meanreturn = mean(monthly_returns),
            sdreturn = sd(monthly_returns))
```
...
ggplot(myStocks_returns_monthly, aes(x = monthly_returns)) +
   ...
   labs(
     title = "Density Plots for Stocks",
     x = "Stocks",
     y = "Density"
   )
...
The monthly returns for the flatter density plots are more dispersed whereas those with tall peaks are more concentrated. The riskiest stock is EBR and the least risky is SPY due to the shape of their peaks.

Finally, make a plot that shows the expected monthly return (mean) of a stock on the Y axis and the risk (standard deviation) in the X-axis. Please use `ggrepel::geom_text_repel()` to label each stock

```{r risk_return_plot}
# YOUR CODE GOES HERE
ggplot(monthlystocks_summarised, aes(y = meanreturn, x=sdreturn)) +
   geom_point() + ggrepel::geom_text_repel(aes(label = symbol))
...
```{r}
hr_cleaned %>% group_by(attrition)%>% summarise(countYN = count(attrition))

hr_cleaned %>% group_by(years_at_company) %>% summarise(countyears = count(years_at_company))
```
...
```{r}
hr_cleaned %>%  group_by(job_satisfaction)%>% summarise(countjs= count(job_satisfaction),
                                                     percentagejs = countjs/nrow(hr_cleaned)*100) 

hr_cleaned %>%  group_by(work_life_balance)%>% summarise(countwlb= count(work_life_balance),
                                                     percentagewlb = countwlb/nrow(hr_cleaned)*100) 
...
As we can see from the boxplot of males and females and their monthly income, females have a higher median monthly income which could mean that the females in the dataset could be more educated.
The doctors have the highest monthly median income and those with below college education have the least. Doctors also have the highest variability in the IQR. College has the most outliers due to its low standard deviation. 
...
```{r}
hr_cleaned %>% group_by(education) %>%
  summarise(
            medianinc = median(monthly_income),
            meaninc = mean(monthly_income)) %>% ggplot(aes(x=education, y=meaninc)) +
...
# ggplot(hr_cleaned, aes(x = education, y = monthly_income)) + facet_wrap(~education) +
#   geom_boxplot()

hr_cleaned %>% ggplot(aes(x=monthly_income)) +
  geom_histogram()+
  facet_wrap(~education) + theme_wsj()
...
hr_cleaned %>% ggplot(aes(y=monthly_income, x=age)) +
  geom_point()+
  facet_wrap(~job_role) + theme_wsj()
